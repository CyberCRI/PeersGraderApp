		...
		<table class="table">
			...
			<tbody>
				<tr v-for="participant in activity.participants">
					...
					<td v-for="(toReview,index) of participant.reviewed">{{toReview.group}}</td>
				</tr>
			</tbody>
		</table>
	</div>
						...
						if(participant){
							do{ 
									...
									if(shifts[indexShift].pools[indexPool].presenter == '' 
										...
										//&& !shifts[indexShift].pools[indexPool].graders.reduce((a,c)=>a.concat(c.reviewed),[]).reduce((a,c)=>a.concat(c.group),[]).includes(participant.group)
										){
										shifts[indexShift].pools[indexPool].presenter = participant;
										presenters.push(participant);
										addReviewed(indexShift,indexPool,participant);
										...
										&& shifts[indexShift].pools[indexPool].presenter.group != participant.group 
										 && !shifts[indexShift].pools[indexPool].graders.some(x=>x.reviewed.reduce((a,c)=>a.concat(c.group),[]).includes(participant.reviewed[indexShift-1]))
										  && !participant.reviewed.reduce((a,c)=>a.concat(c.group),[]).includes(shifts[indexShift].pools[indexPool].presenter.group)
											&& shifts[indexShift].pools[indexPool].graders.length < sizePool-1){
