    ...
    actions : {
        ...
        setActivity(context){
            if(context.state.activity.urlId) //if activity got an urlId, means the admin wants to modify an existing activity
                return this._vm.$axios.put('/api/activity/'+context.state.activity.urlId,{activity:context.state.activity}).then((response)=>{
                    if(response.data.success){
                        this._vm.$notify({
                            group: 'notifications',
        ...
        getPlanningFinal(context){
            ...
            var shifts = {},
                ...
                nbGrading = nbShifts - context.state.activity.sessions;
            ...
            //rotation algortihm possible si s= (maxStudents-1)²

            function getConstraints(priority){
                //work on that shit and it will be alright
            ...
            var constraints = {
                ...
                isPresentersGroupDifferent : { // applicable si maxStudents > 2
                    check : function (specifier){
                ...
                isGradersGroupDifferent : {// applicable si maxStudents > 2
                    check : function(specifier){
                        ...
                        let ok = false;
                         ...
                         if(specifier.indexShift > 0){
                            ...
                            ok = shifts.students[specifier.indexShift][specifier.indexPool].//chacun de ces graders n'est pas buddy avec student.email
                                                                graders.every(x=>true!==getParticipant(x.email).reviewed.filter((c,i)=>i==specifier.indexPool-1 && c.email!=x.email).map(c=>c.email).includes(specifier.student.email));
                        }
                ...
                isThereABuddy : {// applicable si maxStudents > 2
                    check : function(specifier){
                        ...
                        if(specifier.indexShift > 0){
                            ok = shifts.students[specifier.indexShift][specifier.indexPool].//chacun de ces graders n'est pas buddy avec student.email
                                                                graders.every(x=>!getParticipant(x.email).reviewed.filter(c=>c.email!=x.email).map(c=>c.email).includes(specifier.student.email));
            ...
            console.log('getConstraints',getConstraints(140));
            console.log('shifts');
                        ...
                        let priorityConstraints = getConstraints(priority);
                            ...
                            for(var indexShift = 0; indexShift < shifts.students.length; indexShift++){
                                for(var indexPool = 0; indexPool < shifts.students[indexShift].length; indexPool++){
                                    ...
                                    console.log('Testing to insert',groupStudents[j].email,'at',indexShift,indexPool,'with',priority);
                                    console.log('participant to insert history',getParticipant(groupStudents[j].email))
                                    console.log('shifts context',shifts.students[indexShift][indexPool]);
                                    console.log('participants context presenter',getParticipant(shifts.students[indexShift][indexPool].presenter.email));
                                    console.log('graders')
                                    shifts.students[indexShift][indexPool].graders.forEach(x=>{console.log(getParticipant(x.email));});
