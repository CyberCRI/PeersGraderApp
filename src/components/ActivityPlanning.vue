		...
		<table class="table">
			...
			<tbody>
				<tr v-for="participant in activity.participants">
					...
					<td v-for="(toReview,index) of participant.reviewed">{{toReview}}</td>
				</tr>
			</tbody>
		</table>
	</div>
    	...
    	maxSizePool(groups){
			...
			getPlanning(){
				...
				*/
				function addReviewed(indexShift,indexPool,participant){
					if(shifts[indexShift].pools[indexPool].presenter!=''){
						...
						vm.activity.participants[indexParticipant].reviewed.push(shifts[indexShift].pools[indexPool].presenter.group);
					}
				}
				...
				*/
						...
						if(participant){
							do{ 
									...
									if(shifts[indexShift].pools[indexPool].presenter == '' 
										...
										&& !shifts[indexShift].pools[indexPool].graders.reduce((a,c)=>a.concat(c.reviewed),[]).includes(participant.group)){
										shifts[indexShift].pools[indexPool].presenter = participant;
										presenters.push(participant);
										addReviewed(indexShift,indexPool,participant);
										addReviewers(indexShift,indexPool)
										...
										&& shifts[indexShift].pools[indexPool].presenter.group != participant.group 
										 && !shifts[indexShift].pools[indexPool].graders.some(x=>x.reviewed.includes(participant.reviewed[indexShift-1]))
										  && !participant.reviewed.includes(shifts[indexShift].pools[indexPool].presenter.group)
											&& shifts[indexShift].pools[indexPool].graders.length < sizePool-1){
										...
										if(!shifts[indexShift].pools[indexPool].graders.find(e=>e.group==participant.group) 
										&& shifts[indexShift].pools[indexPool].presenter.group != participant.group 
										&& !shifts[indexShift].pools[indexPool].graders.some(x=>x.reviewed.includes(participant.reviewed[indexShift-1]))
										  && !participant.reviewed.includes(shifts[indexShift].pools[indexPool].presenter.group)
											&& shifts[indexShift].pools[indexPool].graders.length < sizePool-1){
				...
				console.log('graders')
				console.log(Object.values(getGraders(0)))
